...
sudo su - "$dendro_user_name" -c "source ~/.bashrc; cd $dendro_installation_path && npm install" || die "Unable to install npm dependencies at $dendro_installation_path as $dendro_user_name"
